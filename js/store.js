...
// Типы работ по умолчанию (с цветом и текстом по умолчанию)
const DEFAULT_TYPES = {
    'Транспорт': { color: '#5da7ff', defaultText: 'Доставка оборудования' },
    'Разгрузка': { color: '#90be6d', defaultText: 'Разгрузка техники' },
    'Погрузка': { color: '#80ffea', defaultText: 'Погрузка оборудования' },
    'Монтаж': { color: '#ffd166', defaultText: 'Монтаж конструкции' },
    'Демонтаж': { color: '#ff6b6b', defaultText: 'Демонтаж конструкции' },
    'Пуско-наладка': { color: '#f9c74f', defaultText: 'Пуско-наладочные работы' },
    'Уборка': { color: '#bdb2ff', defaultText: 'Уборка территории' },
    'Репетиция': { color: '#a0c4ff', defaultText: 'Репетиция' },
    'Веха': { color: '#d56cff', defaultText: 'Веха' }
};
...
class Store {
    constructor() {
        ...
        // Подписчики на изменения (паттерн Observer)
        this.observers = [];
        
        // Подписчики для рассылки (Telegram chat_id)
        this.telegramSubscribers = this.loadTelegramSubscribers();
    }
    ...
    loadTypes() {
        try {
            ...
            if (saved) {
                ...
                // Конвертируем старый формат (просто цвет) в новый
                const result = {};
                for (const [name, value] of Object.entries(parsed)) {
                    if (typeof value === 'string') {
                        result[name] = { color: value, defaultText: '' };
                    } else {
                        result[name] = value;
                    }
                }
            ...
            return JSON.parse(JSON.stringify(DEFAULT_TYPES));
        } catch {
            return JSON.parse(JSON.stringify(DEFAULT_TYPES));
        }
    }
    ...
    loadTelegramSubscribers() {
        try {
            const saved = localStorage.getItem('telegram_subscribers');
            return saved ? JSON.parse(saved) : [];
        } catch {
            return [];
        }
    ...
    saveTelegramSubscribers() {
        localStorage.setItem('telegram_subscribers', JSON.stringify(this.telegramSubscribers));
    }

    addTelegramSubscriber(name, chatId) {
        const exists = this.telegramSubscribers.find(s => s.chatId === chatId);
        if (!exists) {
            this.telegramSubscribers.push({ 
                id: Date.now(), 
                name: name, 
                chatId: chatId 
            });
            this.saveTelegramSubscribers();
            this.notify('subscribers-change');
            return true;
        }
        return false;
    }

    removeTelegramSubscriber(id) {
        const index = this.telegramSubscribers.findIndex(s => s.id === id);
        if (index !== -1) {
            this.telegramSubscribers.splice(index, 1);
            this.saveTelegramSubscribers();
            this.notify('subscribers-change');
            return true;
        }
    ...
    getTelegramSubscribers() {
        return [...this.telegramSubscribers];
    }
    ...
    // Добавить задачу
    add(task) {
        // Если тип задан и есть текст по умолчанию - используем его
        let taskText = task.task || 'Новая работа';
        if (task.type && this.types[task.type]?.defaultText && !task.task) {
            taskText = this.types[task.type].defaultText;
        }
        ...
        const newTask = {
            ...
            color: task.color || this.types[task.type]?.color || '#5da7ff'
        };
    ...
    update(id, fields) {
        ...
        if (task) {
            // Если меняется тип и задача ещё не редактировалась - обновляем текст
            if (fields.type && fields.type !== task.type) {
                const typeInfo = this.types[fields.type];
                if (typeInfo) {
                    fields.color = typeInfo.color;
                    // Если текущий текст совпадает с текстом старого типа или пустой - меняем
                    const oldTypeInfo = this.types[task.type];
                    if (!task.task || task.task === 'Новая работа' || 
                        (oldTypeInfo && task.task === oldTypeInfo.defaultText)) {
                        fields.task = typeInfo.defaultText || task.task;
                    }
                }
            }
    ...
    getTypeColor(typeName) {
        return this.types[typeName]?.color || '#5da7ff';
    }

    getTypeDefaultText(typeName) {
        return this.types[typeName]?.defaultText || '';
    }

    addType(name, color = '#5da7ff', defaultText = '') {
        this.types[name] = { color, defaultText };
        localStorage.setItem('gantt_types', JSON.stringify(this.types));
        this.notify('types-change');
    }

    updateType(oldName, newName, color, defaultText) {
        if (oldName !== newName) {
        ...
        this.types[newName] = { color, defaultText };
        localStorage.setItem('gantt_types', JSON.stringify(this.types));
        this.notify('types-change');
    }
    ...
    notify(action, data) {
        this.observers.forEach(cb => cb(action, data));
    }
