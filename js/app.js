...
class App {
    ...
    handleKeyboard(e) {
        ...
        // 1-3 - переключение вкладок
        if (!e.ctrlKey && !e.altKey) {
            const tabs = { '1': 'table', '2': 'gantt', '3': 'settings' };
            if (tabs[e.key]) {
    ...
    renderTypeSettings() {
        ...
        Object.entries(types).forEach(([name, color]) => {
            ...
            item.innerHTML = `
                <input type="text" value="${escapeHtml(name)}" data-old="${escapeHtml(name)}">
                <input type="color" value="${color}">
                <button class="btn ghost delete-type-btn" style="padding:4px 8px">✕</button>
            `;
            ...
            item.querySelector('input[type="color"]').onchange = (e) => {
                ...
                store.updateType(nameInput.dataset.old, nameInput.value, e.target.value);
                nameInput.dataset.old = nameInput.value;
            };

            // Изменение названия
            item.querySelector('input[type="text"]').onchange = (e) => {
                const colorInput = item.querySelector('input[type="color"]');
                store.updateType(e.target.dataset.old, e.target.value, colorInput.value);
