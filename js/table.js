...
export class Table {
    ...
    createRow(task) {
        ...
        const currentColor = types[task.type]?.color || task.color || '#5da7ff';
        const typeOptions = Object.entries(types).map(([typeName, typeInfo]) => 
            `<option value="${escapeHtml(typeName)}" 
                data-color="${typeInfo.color}"
        ...
        tr.querySelectorAll('input, select').forEach(input => {
            ...
            if (field === 'type') {
                input.addEventListener('change', (e) => {
                    ...
                    const newColor = selectedOption.dataset.color || store.getTypeColor(e.target.value) || '#5da7ff';
                    ...
                    // Обновляем текст задачи если есть defaultText
                    const defaultText = store.getTypeDefaultText(e.target.value);
                    if (defaultText) {
                        const taskInput = tr.querySelector('[data-field="task"]');
                        const currentTask = store.getById(task.id);
                        if (taskInput && (!currentTask.task || currentTask.task === 'Новая работа' || 
                            currentTask.task === store.getTypeDefaultText(currentTask.type))) {
    ...
    saveRow(id, tr) {
        ...
        if (fields.type) {
            fields.color = store.getTypeColor(fields.type);
        }
