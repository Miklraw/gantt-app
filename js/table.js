...
export class Table {
    ...
    createRow(task) {
        ...
        // Генерируем options для типов с цветами
    ...
    addTask(milestone = false) {
        ...
        // Проверяем настройку "Продолжать от последней работы"
        if (store.getSetting('continueFromLast')) {
        ...
        // Если не нашли или настройка выключена - используем текущее время
        if (!startDate) {
            startDate = new Date();
